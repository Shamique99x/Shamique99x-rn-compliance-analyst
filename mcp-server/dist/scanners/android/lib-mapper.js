...
exports.resolveUpgrades = resolveUpgrades;
...
function normalise(soName) {
    return soName
        .toLowerCase()
        .replace(/^lib/, "")
        .replace(/\.so(\.[^.]+)?$/, "");
}
function readProjectDeps(projectPath) {
 ...
 */
async function resolveUpgrades(nonCompliantLibs, projectPath) {
    const db = loadDb();
    ...
    for (const lib of nonCompliantLibs) {
        ...
        const mapping = db.mappings.find((m) => m.so_patterns.some((p) => norm.includes(p.toLowerCase())));
        if (!mapping) {
        ...
        const currentVer = rawVersion
            ? (semver_1.default.coerce(rawVersion)?.version ?? rawVersion)
            : "not installed";
        // Already compliant — skip silently
        if (currentVer !== "not installed" &&
            semver_1.default.valid(currentVer) &&
            semver_1.default.gte(currentVer, mapping.min_version)) {
            continue;
        }
    ...
    if (unknownLibs.length > 0) {
        ...
        for (const lib of unknownLibs) {
            ...
            if (result?.npm_package && result.min_version) {
                ...
                if (currentVer !== "not installed" &&
                    semver_1.default.valid(currentVer) &&
                    semver_1.default.gte(currentVer, result.min_version)) {
                    continue;
                }
                if (!seen.has(packageName)) {
                    seen.set(packageName, {
            ...
            if (!result || result.reason === "Unknown") {
                ...
                if (!seen.has(key)) {
                    seen.set(key, {
                        ...
                        min_version: "unknown",
                        reason: `${lib.name} is not 16 KB page-aligned and could not be matched to a ` +
...
//# sourceMappingURL=lib-mapper.js.map