...
exports.formatScanAsMarkdown = formatScanAsMarkdown;
exports.formatFixAsMarkdown = formatFixAsMarkdown;
const SEVERITY_ICON = {
    error: "🔴",
    warning: "🟡",
    info: "🔵",
};
function formatScanAsMarkdown(result) {
    ...
    if (violations.length === 0) {
        ...
        lines.push(`### Violations (${violations.length} total, ${fixable} auto-fixable)`);
        lines.push("");
    ...
    if (upgrades.length > 0) {
        ...
        for (const u of upgrades) {
            lines.push(`- **${u.name}** — upgrade from \`${u.current_version}\` to \`${u.min_version}\``);
            lines.push(`  ${u.reason}`);
        }
    }
    if (result.apk_inspection) {
        ...
        if (apk.error) {
            ...
            lines.push(`❌ ${apk.non_compliant.length} of ${apk.libraries_checked} libraries are non-compliant.`);
...
function formatFixAsMarkdown(result) {
    const lines = [];
