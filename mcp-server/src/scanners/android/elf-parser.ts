...
export const PAGE_4KB  = 4096;   // 0x1000 — old default, non-compliant
export const PAGE_16KB = 16384;  // 0x4000 — required for Android 15+
 ...
 */
export function parseElf(buf: Buffer): ElfInfo | null {
  ...
  if (!isLE) return null;         // ARM/AArch64 are always LE; skip exotic BE targets
  ...
  if (is64bit) {
    ...
    if (rawPhoff > BigInt(buf.length)) return null; // sanity: offset can't exceed file size
  ...
  if (phnum === 0 || phoff === 0) return null;
  ...
  if (loadAlignments.length === 0) return null;
